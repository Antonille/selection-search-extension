...
const findings = [];
let repoState = { gitAvailable: false, insideRepo: false };
...
function detectRepoState() {
  const gitVersion = spawnSync('git', ['--version'], { stdio: 'ignore' });
  if (gitVersion.status !== 0) {
    return { gitAvailable: false, insideRepo: false };
  }
  ...
  return {
    gitAvailable: true,
    insideRepo: probe.status === 0 && String(probe.stdout || '').trim() === 'true'
  };
}

function scanGitHistory() {
  const objectsRaw = execFileSync('git', ['rev-list', '--objects', '--all'], {
    encoding: 'utf8',
  ...
  for (const line of objectsRaw.split(/\r?\n/)) {
    ...
    try {
      const buf = execFileSync('git', ['cat-file', '-p', oid], {
        encoding: null,
...
repoState = detectRepoState();
walk(root);
if (scanHistory && repoState.insideRepo) {
  scanGitHistory();
}
...
console.log(scanHistory && repoState.insideRepo
